	...
	public function executeAction(): ResponseInterface
	{
		...
		$registerResult = $this->registerHandler->register(
			CustomerRegisterData::create()
				->setEmail($values->get(RegisterData::EMAIL)
					->getValue())
				->setPassword($values->get(RegisterData::PASSWORD)
					->getValue())
				->setPasswordVerify($values->get(RegisterData::PASSWORD_VERIFY)
					->getValue())
				->setSalutation($values->get(RegisterData::SALUTATION)
					->getValue())
				->setTitle(
				...
				->setFirstName($values->get(RegisterData::FIRST_NAME)
					->getValue())
				->setLastName($values->get(RegisterData::LAST_NAME)
					->getValue())
				->setCompany(
